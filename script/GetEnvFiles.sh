...
function get_envFiles(){
  ...
  note "Setting up your enviorment...\n"
    ...
    if [ ! -e $HOME/zsh-completions ]; then
      ...
      printf "${cyan}zsh-completions ready.${normal}\n"
    else
      progress sleep 0.5 
      printf "${cyan}zsh-completions already installed.${normal}\n"      
    fi
    ...
    if [ ! -e $HOME/.vim ]; then
      ...
      printf "${cyan}vim ready.${normal}\n"
    else
      progress sleep 0.5
      printf "${cyan}vim already installed.${normal}\n"
    fi
